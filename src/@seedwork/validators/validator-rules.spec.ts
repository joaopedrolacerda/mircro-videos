...
describe('Validator Rules Unit Tests', () => {
  ...
  test('required validation rule', () => {
    ...
    expect( () => ValidatorRules.values(item.value, item.property).required()).toThrow(new ValidationError(item.messageError))
    })
    arrange = [
    ...
    arrange.forEach(item => {
      expect( () => ValidatorRules.values(item.value, item.property).required()).not.toThrow(new ValidationError(item.messageError))
      })
  })
})
